    ...
    def handle_rx(self, data: bytearray):
        ...
        match data[0]:
            ...
            case 3: # contact
                ...
                # Modified decode with error handling
                try:
                    c["adv_name"] = data[100:132].decode().replace("\0","")
                except UnicodeDecodeError:
                    # Fallback options:
                    # Option 1: Use a different encoding
                    try:
                        c["adv_name"] = data[100:132].decode('latin-1').replace("\0","")
                    except Exception:
                        # Option 2: Use hexadecimal representation if all else fails
                        c["adv_name"] = f"unnamed-{data[100:132].hex()[:8]}"
